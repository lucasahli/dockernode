...
export class RedisRepository implements LoginRepository, UserRepository, ReminderRepository {
    ...
    async addLogin(email: string, password: string, associatedUserIds: string[]): Promise<Login> {
        ...
        await this.redis.hSet(
            ...
            [...Object.entries({email: email, password: password, auth: authsecret}).flat()]
        );
        ...
        return Promise.resolve(new Login(loginId.toString(), email, password, associatedUserIds));
    }

    async addReminder(title: string, date: Date, ownerId: string): Promise<Reminder> {
        const reminderId = await this.redis.incr("next_reminder_id");
        console.log("DATE: ", date);
        await this.redis.hSet(
            "reminder:" + reminderId.toString(),
            [...Object.entries({title: title, date: date.toISOString(), ownerId: ownerId}).flat()]
        );
        await this.redis.sAdd("user:" + ownerId + "reminders", reminderId.toString());
        return Promise.resolve(new Reminder(reminderId.toString(), title, date, ownerId));
    }

    async addUser(login: Login, role: UserRole, firstname: string, lastname: string): Promise<User> {
        ...
        await this.redis.hSet(
            ...
            [...Object.entries({login: login.id, role: role, firstname: firstname, lastname: lastname}).flat()]
        );
        await this.redis.sAdd("login:" + login.id + "associated_user_ids", userId.toString());
        return Promise.resolve(new User(userId.toString(), login, role, firstname, lastname));
    }
    ...
    async deleteLogin(id: string): Promise<boolean> {
        ...
        // await this.redis.sAdd("login:" + loginId.toString() + "associated_user_ids", associatedUserIds);
        const associatedUserIds = await this.redis.sMembers("login:" + id + "associated_user_ids");
        await this.redis.sRem("login:" + id + "associated_user_ids", associatedUserIds);
    ...
    async deleteReminder(id: string): Promise<boolean> {
        ...
        const nbrOfDeletedFields = await this.redis.hDel("reminder:" + id, allFields);
        return Promise.resolve(nbrOfDeletedFields > 0);
    }
    ...
    async deleteUser(id: string): Promise<boolean> {
        ...
        await this.redis.sRem("login:" + userData.login + "associated_user_ids", id);
    ...
    getLoginByEmail(email: string): Promise<Login | null> {
        return new Promise<Login | null>(async (resolve, reject) => {
            ...
            return resolve(new Login(loginId, loginData.email, loginData.password, associatedUserIds));
        });
    }
    ...
    getLoginById(id: string): Promise<Login | null> {
        return new Promise<Login | null>(async (resolve, reject) => {
            ...
            const associatedUserIds = await this.redis.sMembers("login:" + id + "associated_user_ids");
            if(loginData) {
                return resolve(new Login(id, loginData.email, loginData.password, associatedUserIds));
            }
            else {
    ...
    getReminderById(id: string): Promise<Reminder | null> {
        return new Promise<Reminder | null>(async (resolve, reject) => {
            const reminderData = await this.redis.hGetAll("reminder:" + id);
            if(reminderData) {
                return resolve(new Reminder(id, reminderData.title, new Date(reminderData.date), reminderData.ownerId));
            }
            else {
    ...
    getUserById(id: string): Promise<User | null> {
        return new Promise<User | null>(async (resolve, reject) => {
            ...
            if(userData && login) {
                return resolve(new User(id, login, userData.role as UserRole, userData.firstname, userData.lastname));
            }
            else {
    ...
    getRemindersByUser(userId: string): Promise<Reminder[] | null> {
        return new Promise<Reminder[] | null>( async (resolve, reject) => {
            ...
            const reminderIds = await this.redis.sMembers("user:" + userId + "reminders");
